...
import { useMusic } from "@/components/MusicProvider"; // Import hook untuk sinkronisasi tab navbar
...
export default function AboutPPLG2() {
  ...
  // Mengambil fungsi setActiveTab dari Global MusicProvider
  const { setActiveTab } = useMusic();

  // 1. Sinkronisasi Tab Active & Fetch Data Supabase
  useEffect(() => {
    // Beritahu navbar bahwa saat ini user sedang membuka halaman About
    setActiveTab("About");
  ...
  // 2. Logic Autoplay Carousel (Bergeser otomatis ke samping setiap 4 detik)
  useEffect(() => {
    if (items.length <= 1) return;

    const interval = setInterval(() => {
      setDirection(1); // Arah geser ke kanan
        ...
        <div className="max-w-4xl mx-auto px-4 md:px-6 flex flex-col items-center">
          ...
          <div className="w-full text-center space-y-6 mb-12">
            ...
            <div className="space-y-2">
              ...
              {/* Tambahan Kalimat Angkatan 60 */}
              <motion.p
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ delay: 0.1 }}
                className="text-sm md:text-lg font-bold text-indigo-500 font-mono uppercase tracking-widest"
              >
                — Angkatan 60 —
              </motion.p>
            </div>
          ...
          {/* Autoplay & Animate Slider Frame */}
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="w-full relative mb-16"
          >
            {/* Dekorasi Shadow Soft di belakang frame gambar */}
            ...
            <div className="w-full h-[320px] sm:h-[450px] md:h-[520px] bg-slate-900 rounded-[2.5rem] md:rounded-[3.5rem] overflow-hidden border-4 border-white shadow-2xl relative flex items-center justify-center">
              ...
              <AnimatePresence initial={false} custom={direction} mode="popLayout">
                {currentItem?.image_url ? (
                  <motion.div
                    ...
                    custom={direction}
                    variants={{
                      enter: (dir) => ({ x: dir > 0 ? 200 : -200, opacity: 0, filter: "blur(4px)" }),
                      center: { x: 0, opacity: 1, filter: "blur(0px)" },
                      exit: (dir) => ({ x: dir < 0 ? 200 : -200, opacity: 0, filter: "blur(4px)" }),
                    }}
                    initial="enter"
                    animate="center"
                    exit="exit"
              ...
              {/* Lapisan Gradient agar teks deskripsi bawah terbaca jelas */}
              ...
              {/* Kontrol Deskripsi & Dot Navigator */}
              ...
              {/* Tombol Manual Panah Kiri dan Kanan */}
              {items.length > 1 && (
                <>
        ...
        <div className="max-w-6xl mx-auto px-4 md:px-6 flex flex-col md:flex-row items-center justify-between gap-4 text-center md:text-left">
          <div className="space-y-1">
            <p className="text-xs font-bold text-slate-800">
              © {new Date().getFullYear()} PPLG 2 — Angkatan 60. All Rights Reserved.
            </p>
